...
```shell
# 执行导表
python main.py --export my_config.json
```

导表的时候，需要指定一个json格式的配置文件，描述了导表过程中需要的一些参数。目前，导表工具支持3种表格配置方式：
...
## CONVENTION_TABLE
Excel与转换器对应关系表。该参数位于`convention_table.py`文件中。每个数组元素也是一个数组，描述了Excel与转换器的对应关系。
